...
# 第二节  自动计算微分（Differentiation）

# 一个使用 MNIST 数据集训练网络的示例，使用 GradientDescentOptimizer。
...
# non-linear sigmod function at each neuron
def sigmoid(x):
    sigma = tf.div(tf.constant(1.0), tf.add(tf.constant(1.0), tf.exp(tf.negative(x))))
    return sigma

# starting from first layer with w * x + b, then apply sigmoid to add nonlinearity
...
for i in range(10000):
    ...
    if i % 1000 == 0:
        res = sess.run(acct_res, feed_dict={x: data.test.images[:1000], y: data.test.labels[:1000]})
        print(res)